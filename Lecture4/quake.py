...
for i in xrange(len(log10N_values)):
    if M_values[i]>3 :
     M_value.append(M_values[i])
     log10N_value.append(log10N_values[i])
print 'log10N_values is '
for i in xrange(len(log10N_value)) :
    print str(M_value[i]) + ' ' + str(log10N_value[i])
# First plot our "home-grown" values with our least-squares
# fit in place.
plt.subplot( 2, 1, 1)
plt.plot( M_value, log10N_value, 'v')
plt.xlabel( 'Magnitude (M)' )
plt.ylabel( 'log(N)' )
# Next also plot matplotlib's version of the same thing.
plt.subplot( 2, 1, 2)
plt.hist( magvalues, bins=90, range=[3.0,10.0], log=True, bottom=0.1,cumulative=-1)
plt.xlabel( 'Magnitude (M)' )
plt.ylabel( 'N' )
# perform a least square fit
fit = least_squares_fit(M_value, log10N_value)
print ' least_squares fit to data where magnitude > 3:'
