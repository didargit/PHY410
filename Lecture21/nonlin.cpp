...
public : 
  ...
  static constexpr double g=9.8;          // acceleration of gravity
...
;
 ...
 int main() {
   ...
   //always use Adaptive RK4
   int method = 2;
   ofstream bifur("bifurication_plot.txt");
 for ( int q=0; q <31 ;q++) {					   
   // setting initial position velocity and other vaiables
   const double pi = 3.141592654;
   double theta = 15*pi/180;   // Convert angle to radians
   double omega = 5*pi/180.0;
   double gamma = .5;
   double omega_D = .6667;
   ...
   */
   for( i=1; i< poincare_map.size(); i++ ) {
     bifur << F_D << " "  << poincare_map[i].second << endl;
   }
  }
   bifur.close();
